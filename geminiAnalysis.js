...
Завдання — сформуй стислий підсумок для Telegram-чату. Звичайний текст із символами • та emoji; для виділення дозволені ЛИШЕ HTML-теги <b>…</b> (жирний) та <i>…</i> (курсив). Жодного markdown (**зірочки**, __підкреслення__ тощо) — він не відображається.

Структура відповіді (використовуй точно такий формат; заголовки розділів — жирним):
...
📢 <b>Відбій:</b> [оголошено / не оголошено]
...
Формат: звичайний текст, до 100 слів, можна emoji та символ •. Для виділення дозволені ЛИШЕ HTML-теги <b>…</b> та <i>…</i> (назву регіону й головну загрозу — жирним); жодного markdown (зірочок, підкреслень). Відповідай українською.`;
}
...
- Звичайний текст, emoji та • дозволені. Для виділення дозволені ЛИШЕ HTML-теги <b>…</b> та <i>…</i>; жодного markdown (зірочок, підкреслень). До 120 слів.

Формат (заголовок і назви пунктів — жирним, як показано):
🌙 <b>${regionName} — за ніч</b>

• <b>Що летіло:</b> [типи та кількості над регіоном / поблизу, з приблизним часом]
• <b>Пуски та події:</b> [пуски БпЛА з кількістю, зліт авіації, балістика, «Калібри» — з часом]
• <b>Зараз:</b> [одним реченням, що каже останнє повідомлення або NEPTUN]`;
}
